...
def trainer(conf, trial=False, verbose=True):
    ...
    for epoch in list(range(epochs)):
        ...
        if (epoch == 0 and region == "fixed") or (region == "random") or (region == "quasi") or (
            epoch == 0 and region == "global") or (region == "quasi_global") or (
            epoch == 0 and region == "global1") or (epoch == 0 and region == "global2"):
            ...
            valid_loader = DataLoader(
                valid, batch_size=valid_batch_size, shuffle=True,
                drop_last=True, worker_init_fn=seed_worker, generator=g
            )
            tests_loader = DataLoader(
                tests, batch_size=valid_batch_size, shuffle=False,
